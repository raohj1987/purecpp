...
class APIService {
    ...
    // 编辑文章
    async editArticle(articleId, title, content, tags) {
        return this.request('/api/v1/edit_article', {
            method: 'POST',
            body: JSON.stringify({article_id: articleId, title, content, tags})
        });
    }
    ...
    // 审核文章
    async reviewArticle(articleId, isApproved, comment) {
        return this.request('/api/v1/review_pending_article', {
            method: 'POST',
            body: JSON.stringify({article_id: articleId, is_approved: isApproved, comment})
        });
    }
