...
public:
    ...
    static UserLevel calculate_level(uint64_t experience) {
        ...
        if (level_rules.empty()) {
            // 默认等级规则，保持原逻辑
            if (experience >= 38400) return UserLevel::LEVEL_10;
            if (experience >= 19200) return UserLevel::LEVEL_9;
            if (experience >= 9600) return UserLevel::LEVEL_8;
            if (experience >= 4800) return UserLevel::LEVEL_7;
            if (experience >= 2400) return UserLevel::LEVEL_6;
            if (experience >= 1200) return UserLevel::LEVEL_5;
            if (experience >= 600) return UserLevel::LEVEL_4;
            if (experience >= 300) return UserLevel::LEVEL_3;
            if (experience >= 100) return UserLevel::LEVEL_2;
    ...
    static uint64_t get_required_experience(UserLevel current_level) {
        ...
        if (level_rules.empty()) {
            ...
            switch (current_level) {
                case UserLevel::LEVEL_1: return 100;
                case UserLevel::LEVEL_2: return 300;
                case UserLevel::LEVEL_3: return 600;
                case UserLevel::LEVEL_4: return 1200;
                case UserLevel::LEVEL_5: return 2400;
                case UserLevel::LEVEL_6: return 4800;
                case UserLevel::LEVEL_7: return 9600;
                case UserLevel::LEVEL_8: return 19200;
                case UserLevel::LEVEL_9: return 38400;
                case UserLevel::LEVEL_10: return 0; // 已达最高等级
                default: return 0;
            }
        }
    ...
    static uint64_t get_level_experience_min(UserLevel current_level) {
        ...
        if (level_rules.empty()) {
            // 默认等级规则，保持原逻辑
            switch (current_level) {
                case UserLevel::LEVEL_1: return 0;
                case UserLevel::LEVEL_2: return 100;
                case UserLevel::LEVEL_3: return 300;
                case UserLevel::LEVEL_4: return 600;
                case UserLevel::LEVEL_5: return 1200;
                case UserLevel::LEVEL_6: return 2400;
                case UserLevel::LEVEL_7: return 4800;
                case UserLevel::LEVEL_8: return 9600;
                case UserLevel::LEVEL_9: return 19200;
                case UserLevel::LEVEL_10: return 38400;
                default: return 0;
            }
        }
     ...
     * @return 进度百分比（0-100）
     */
    static int calculate_level_progress(uint64_t experience, UserLevel current_level) {
        if (current_level == UserLevel::LEVEL_10) return 100;
        
        uint64_t min_exp = get_level_experience_min(current_level);
        uint64_t next_level_min = get_level_experience_min(static_cast<UserLevel>(static_cast<int>(current_level) + 1));
        uint64_t level_range = next_level_min - min_exp;
        uint64_t current_in_level = experience - min_exp;
        
        return static_cast<int>((static_cast<double>(current_in_level) / level_range) * 100);
    }
     ...
     * @return 是否可以增加经验值
     */
    static bool check_experience_limit(uint64_t user_id, int64_t experience_add, PointChangeType change_type) {
        ...
        // 使用ORMpp的查询方式获取总经验值
        auto total_experience = conn->select(sum(col(&user_experience_detail_t::experience_change)))
    ...
    static bool add_experience(uint64_t user_id, uint64_t experience_add) {
        // 默认使用系统奖励类型
        return add_experience(user_id, experience_add, PointChangeType::SYSTEM_REWARD);
    }
     ...
     * @return 操作是否成功
     */
    static bool add_experience(uint64_t user_id, int64_t experience_add, PointChangeType change_type, 
     ...
     * @return 操作是否成功
     */
    static bool reduce_experience(uint64_t user_id, int64_t experience_reduce, PointChangeType change_type, 
                              std::optional<uint64_t> related_id = std::nullopt,
                              std::optional<std::string> related_type = std::nullopt,
                              std::optional<std::string> description = std::nullopt) {
        auto conn = connection_pool<dbng<mysql>>::instance().get();
    ...
    static bool purchase_privilege(uint64_t user_id, uint64_t privilege_id) {
        ...
        auto privileges = conn->select(ormpp::all)
                            .from<privileges_t>()
                            .where(col(&privileges_t::id).param() && col(&privileges_t::is_active).param())
        ...
        // 减少用户经验值
        if (!reduce_experience(user_id, privilege.points_cost, PointChangeType::PURCHASE_PRIVILEGE, 
                           privilege_id, "privilege", "购买特权：" + privilege.name)) {
            conn->rollback();
            return false;
        }
        ...
        user_privileges_t user_privilege{
            .user_id = user_id,
     ...
     * @return 操作是否成功
     */
    static bool gift_user(uint64_t sender_id, uint64_t receiver_id, int64_t experience_amount,
        ...
        // 减少打赏者经验值
        if (!reduce_experience(sender_id, experience_amount, PointChangeType::GIFT_TO_USER, 
                           article_id, "gift", "打赏用户")) {
            conn->rollback();
            return false;
        }

        // 增加接收者经验值
        if (!add_experience(receiver_id, experience_amount, PointChangeType::SYSTEM_REWARD, 
                        article_id, "gift", "收到打赏")) {
            conn->rollback();
            return false;
        }
...
public:
    ...
    void get_user_level(coro_http_request &req, coro_http_response &resp) {
        ...
        if (user_id_str.empty()) {
            resp.set_status_and_content(status_type::bad_request, make_error("用户未登录"));
            return;
        }
        ...
        if (!user_level_t::get_user_level_info(user_id, user_info)) {
            resp.set_status_and_content(status_type::internal_server_error, make_error("获取用户信息失败"));
            return;
        }

        // 计算等级进度
        int level_progress = user_level_t::calculate_level_progress(user_info.experience, user_info.level);
        uint64_t next_level_required = user_level_t::get_required_experience(user_info.level);

        // 构建响应数据
        user_level_info resp_data{
            .user_id = user_info.id,
            .username = std::string(user_info.user_name.data()),
        ...
        resp.set_status_and_content(status_type::ok, make_data(resp_data, "获取用户等级信息成功"));
    }
     ...
     * @param resp HTTP响应
     */
    void get_experience_transactions(coro_http_request &req, coro_http_response &resp) {
        // 从请求中获取用户ID
        auto user_id_str = req.get_header_value("X-User-ID");
        if (user_id_str.empty()) {
            resp.set_status_and_content(status_type::bad_request, make_error("用户未登录"));
        ...
        auto conn = connection_pool<dbng<mysql>>::instance().get();
        if (conn == nullptr) {
            resp.set_status_and_content(status_type::internal_server_error, make_error("数据库连接失败"));
            return;
        }

        // 计算总记录数
        auto total_count = conn->select(count(col(&user_experience_detail_t::id)))
                            .from<user_experience_detail_t>()
                            .where(col(&user_experience_detail_t::user_id).param())
                            .collect(user_id);
        ...
        auto transactions = conn->select(ormpp::all)
                            .from<user_experience_detail_t>()
                            .where(col(&user_experience_detail_t::user_id).param())
        ...
        experience_transactions_resp resp_data{
        ...
        resp.set_status_and_content(status_type::ok, make_data(resp_data, "获取经验值交易记录成功"));
    }
    ...
    void purchase_privilege(coro_http_request &req, coro_http_response &resp) {
        ...
        if (ec) {
            resp.set_status_and_content(status_type::bad_request, make_error("请求参数无效"));
            return;
        }

        // 购买特权
        if (!user_level_t::purchase_privilege(user_id, info.privilege_id)) {
            resp.set_status_and_content(status_type::bad_request, make_error("购买特权失败，可能是积分不足或特权不存在"));
            return;
        }

        resp.set_status_and_content(status_type::ok, make_data(empty_data{}, "购买特权成功"));
    }
    ...
    void user_gifts(coro_http_request &req, coro_http_response &resp) {
        ...
        // 打赏用户
        if (!user_level_t::gift_user(sender_id, info.receiver_id, info.points_amount, 
                                    info.article_id, info.comment_id, info.message)) {
            resp.set_status_and_content(status_type::bad_request, make_error("打赏失败，可能是积分不足或接收者不存在"));
            return;
        }

        resp.set_status_and_content(status_type::ok, make_data(empty_data{}, "打赏成功"));
    }
     ...
     * @param resp HTTP响应
     */
    void get_available_privileges(coro_http_request &req, coro_http_response &resp) {
        auto conn = connection_pool<dbng<mysql>>::instance().get();
        if (conn == nullptr) {
            resp.set_status_and_content(status_type::internal_server_error, make_error("数据库连接失败"));
            return;
        ...
        resp.set_status_and_content(status_type::ok, make_data(privileges, "获取可用特权列表成功"));
    }
};
}